...
ui <- miniPage(
  ...
  miniButtonBlock(
    div(
      ...
      selectInput("h", "Hypothesis", choices = H0, selectize = FALSE, selected = 3, width = "100%"),
      sliderInput("a", "\\(a\\)-level", 0.001, 0.2, 0.05, 0.001, ticks = FALSE, width = "100%"),
      sliderInput("df", "Degrees of freedom", DF$MIN, DF$MAX, DF$DEFAULT, ticks = FALSE, width = "100%"),
    ),
    div(
      style = "display: flex; flex-flow: column nowrap; flex: auto 1 1;",
...
server <- function(input, output, session) {
  ...
  critical_values <- reactive({
    ...
    d <- dt(t, input$df)
    p <- c(pt(t[1], input$df), pt(t[2], input$df, lower.tail = FALSE))
    tibble(t, d, p)
  })

  output$critical_value_upper <- renderPrint({
    withMathJax(span(
    glue("\\(P(T_{{{df}}} > t) = {p}, \\quad t = {t}\\)", df = input$df, t = critical_values()$t[2] %>% round(3), p = critical_values()$p[2] %>% round(3))))
  })

  output$critical_value_lower <- renderPrint({
    withMathJax(span(
    glue("\\(P(T_{{{df}}} ≤ t) = {p}, \\quad t = {t}\\)", df = input$df, t = critical_values()$t[1] %>% round(3), p = critical_values()$p[1] %>% round(3))))
  })
  ...
  output$distPlot <- renderPlot({
    ...
    if (input$h != 1) {
      ...
      plot <- plot + 
        geom_segment(x = val$t, xend = val$t, y = 0, yend = val$d, colour = COLOUR) + 
        geom_area(data = data() %>% filter(t > val$t), fill = COLOUR, alpha = 0.2) + 
        annotate(
          "text", val$t, val$d, parse = TRUE,
          label = glue("P(T[{df}]>{t})=={p}", df = input$df, t = val$t %>% round(3), p = val$p %>% round(3)),
          vjust = 0, hjust = -0.1
        )
    }
