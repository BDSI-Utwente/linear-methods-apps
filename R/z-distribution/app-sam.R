...
label_p_value_html <- function(z) {# Update: adjust label, < instead of <=
    glue(
        "\\(P(Z {ifelse(z <= 0, '<', '>')} {round(z, 2)}) = {ifelse(z <= 0, pnorm(z), 1 - pnorm(z)) %>% abs() %>% round(3)}\\)"
    )
}

label_p_value <- function(z) {# Update: adjust label, < instead of <=
    glue(
        "P(Z {ifelse(z <= 0, '<', '>')} {round(z, 2)}) = {ifelse(z <= 0, pnorm(z), 1 - pnorm(z)) %>% abs() %>% round(3)}"
    )
}
    ...
    output$distribution <- renderPlot({
        ...
        if (drawValue()) {
            plot <- plot +
                ...
                # Update: Not display 2 vertical lines
                # geom_vline(
                ...
                scale_x_continuous(breaks = c(z_lower()%>% round(2), z_upper()%>% round(2)))
        }
        plot
    })
